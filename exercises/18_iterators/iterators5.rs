...
fn count_collection_iterator(collection: &[HashMap<String, Progress>], value: Progress) -> usize {
    ...
    collection
        .iter() // Iterator over each HashMap
        .flat_map(|map| map.values()) // Flatten to iterate over values of each HashMap
        .filter(|&&val| val == value) // Filter values that match the specified `value`
        .count() // Count the number of matching values
}
